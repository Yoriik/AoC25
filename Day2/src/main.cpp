...
bool validateID(long unsigned int j){
    ...
    for(long unsigned int l = 0; l < Id.size()/2; l++){
        if(Id.size()%(l+1)==0){
            int numberOfSubStr = Id.size()/(l+1); //number of substrings of length l+1
            validCnt = 0;
            ...
            for (const char* result = str; (result = std::strstr(result, target)); result += a.size()){
                validCnt++;
            }
            if(validCnt == numberOfSubStr) return false;
        }
    }
    return true;
}
...
int main(){
    ...
    for(int i = 0; i < listSize/2; i++){
        ID1 = IdList[2*i];
        ID2 = IdList[2*i+1];
        for(long unsigned int j = std::stoll(IdList[2*i]); j <= std::stoll(IdList[2*i+1]); j++){
            if(!validateID(j)){
                IdSum += j;
            }
        }
